...
int main()
{
//	CHLMasterDlg dlg( 27010, false, "127.0.0.1" );

	char const *pArgPort = Sys_FindArg( "-port", "27010" );
	char const *pArgLog = Sys_FindArg( "-log", "0" );
	char const *pArgIP = Sys_FindArg( "-ip", "127.0.0.1" );

	int port = atoi( pArgPort );
	bool log = ( atoi( pArgLog ) == 1 );

	CHLMasterDlg dlg( port, log, (char *)pArgIP );
